...
class BatchIndexingOffersInAlgoliaFromDatabaseTest:
    ...
    @mock.patch("pcapi.repos.algolia_indexing.indexing.process_eligible_offers")
    def test_should_index_offers_once_when_offers_per_page_is_one_and_only_one_page(
        self, mock_reindex_offer_ids, mock_get_paginated_active_offer_ids, app
    ):
        # Given
        ...
        assert mock_reindex_offer_ids.call_count == 1
        assert mock_reindex_offer_ids.call_args_list == [mock.call(offer_ids=[1])]
    ...
    def test_should_index_offers_twice_when_offers_per_page_is_one_and_two_pages(
        ...
        assert mock_reindex_offer_ids.call_count == 2
