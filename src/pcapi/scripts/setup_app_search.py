...
# FIXME: double check fields: what is really needed, what is missing?
#  - merge venue_name et venue_public_name?
#  - merge offer's stage_director, speaker, performer?
SCHEMA = {
    # FIXME: do we need "offer_human_id" in native app and/or the
    # webapp? I'd rather compute it from the (raw) id (the "id" key).
    "offer_author": "text",
    "offer_category": "text",
...
def serialize(offer):
    ...
    if venue.longitude is not None and venue.latitude is not None:
        ...
        position = [DEFAULT_LONGITUDE, DEFAULT_LATITUDE]

    return {
        "id": offer.id,
        "offer_author": extra_data.get("author") if extra_data else None,
        "offer_category": offer.offer_category_name_for_app,
        "offer_date_created": offer.dateCreated,
        "offer_dates": dates,
        "offer_description": offer.description,
        "offer_human_id": humanize(offer.id),
        ...
        "offer_music_type": extra_data.get("musicType") if extra_data else None,
        "offer_name": offer.name,
        "offer_performer": extra_data.get("performer") if extra_data else None,
        "offer_prices": [int(stock.price * 100) for stock in offer.bookableStocks],
        "offer_ranking_weight": offer.rankingWeight,
        "offer_show_type": extra_data.get("showSubType") if extra_data else None,
        "offer_speaker": extra_data.get("speaker") if extra_data else None,
        "offer_stage_director": extra_data.get("stageDirector") if extra_data else None,
        "offer_thumb_url": offer.thumbUrl,  # FIXME: return last part of the path only
